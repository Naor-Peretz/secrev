...
TASK-004C | 55c1c4e | 85/85 green; BRIEF §1 verification run LIVE through the Bash tool — heredoc to src/secrev/cli.py refused, sed -i refused; cost measured immediately (gate output can no longer be piped through grep); docs said "not wired" in 3 separate places
TASK-006 | pending | 87/87 green; with .venv/bin/python moved aside the gate exits 2 naming ruff (same state gave exit 0 + "all gates pass" before); pytest branch no longer conflates "no tests" with "no pytest"; remedy string corrected — TASK-014's `pip install -e '.[dev]'` fails until src/ exists
